...
/*   Updated: 2023/08/09 03:15:26 by sumjo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_event_draw(int x, int y, t_param *p)
{
	...
	if (p->mouse_dragging)
	{
		...
		dx = (x - p->mouse_x);
		dy = (y - p->mouse_y);
		init_move_val(p->mod, dx, dy);
		move_image(p->map, p->mod);
		draw(p);
		mlx_put_image_to_window(p->image->mlx, p->image->win, \
		p->image->img, MAR / 2, MAR / 2);
		p->mouse_x = x;
		p->mouse_y = y;
	}
	return (0);
...
int	mouse_event_down(int keycode, int x, int y, t_param *p)
{
	...
	if (keycode == MOUSE_SCROLL_UP || keycode == MOUSE_SCROLL_DOWN)
	{
		...
		if (keycode == MOUSE_SCROLL_UP)
			...
			scale(p, 1.3);
		set_move(p->map, p->mod, p->map->center_x, p->map->center_y);
